...
export const getflightLocation = async (req, res) => {
  ...
  const start = req.query.startDate
    ? moment(req.query.startDate).utc()
    : moment.utc();
  start.set({
    hour: 23,
    minute: 59,
    ...
    for (let i = 0; i < flights.length; i++) {
      ...
      for (let j = paths.length - 1; j >= 0; j--) {
        ...
        if (paths[j].effDate <= start) {
          ...
          flightMap.properties.path = [
            paths[j].deptGeo.coordinates,
            paths[j].arrvGeo.coordinates,
          ];
          ...
          flightMap.properties.pathColor =
            paths[j].serviceType === "Ferry" ? "#55A8FF" : "#FF7A00";
          break;
        }
      }
